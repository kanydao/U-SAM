...
def build_sam_vit_b(num_classes, img_size, checkpoint=None):
    return _build_sam(
...
def load_from(sam, state_dict, image_size, vit_patch_size, encoder_global_attn_indexes):
    ...
    except_keys = ['mask_tokens', 'output_hypernetworks_mlps', 'iou_prediction_head', 'output_upscaling']
    new_state_dict = {k: v for k, v in state_dict.items() if
                      k in sam_dict.keys() and
                      except_keys[0] not in k and
                      except_keys[1] not in k and
                      except_keys[2] not in k and
                      except_keys[3] not in k}
